...
export class NewOnlineBookingModal {
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between border-b border-outline-variant/60 pb-1.5">
                <h4 class="text-xs font-bold uppercase font-label-caps text-primary flex items-center gap-2">
                  ...
                  <span>3. Pre-Collected Identification (Submitted Online / Check-In)</span>
                </h4>
                <span class="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-bold text-[10px] flex items-center gap-1">
                  ...
                  <span id="nob-doc-status-badge">Digital Scan Verified</span>
                </span>
              </div>

              <!-- Document Extraction / OCR Bar -->
              <div class="flex flex-wrap items-center justify-between gap-2 p-2.5 rounded-xl bg-primary/5 border border-primary/20">
                <div class="flex items-center gap-2">
                  <span class="material-symbols-outlined text-[18px] text-primary">document_scanner</span>
                  <div>
                    <span class="text-[11px] font-bold text-primary block">Online Check-In Document Extraction</span>
                    <span class="text-[10px] text-on-surface-variant">Upload or trigger instant optical extraction to populate verified credentials</span>
                  </div>
                </div>
                <div class="flex items-center gap-2">
                  <input type="file" id="nob-doc-file-input" accept="image/*,application/pdf" class="hidden">
                  <button type="button" id="btn-nob-upload-doc" class="px-2.5 py-1.5 rounded-lg border border-primary/30 bg-surface-bright hover:bg-surface-container text-primary font-bold text-[11px] flex items-center gap-1.5 transition-colors cursor-pointer">
                    <span class="material-symbols-outlined text-[14px]">upload_file</span>
                    <span>Upload ID</span>
                  </button>
                  <button type="button" id="btn-nob-extract-doc" class="px-2.5 py-1.5 rounded-lg bg-primary text-on-primary hover:bg-primary/90 font-bold text-[11px] flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                <div>
                  ...
                  <select id="nob-doc-type" class="w-full px-3 py-2 rounded-lg border border-outline-variant bg-surface-bright font-semibold text-xs">
                    ...
                    <option value="AADHAAR" ${this.data.docType === 'AADHAAR' ? 'selected' : ''}>Aadhaar Card</option>
                ...
                <div>
                  <label class="block font-semibold text-on-surface mb-1">Document / ID Number *</label>
  ...
  bindEvents() {
    ...
    // OCR Extraction & Upload triggers
    const fileInput = root.querySelector('#nob-doc-file-input');
    const uploadBtn = root.querySelector('#btn-nob-upload-doc');
    const extractBtn = root.querySelector('#btn-nob-extract-doc');

    uploadBtn?.addEventListener('click', () => fileInput?.click());
    fileInput?.addEventListener('change', (e) => {
      const file = e.target.files?.[0];
      if (file) {
        this.runDocumentExtraction(root.querySelector('#nob-doc-type')?.value, file);
      }
    });

    extractBtn?.addEventListener('click', () => {
      this.runDocumentExtraction(root.querySelector('#nob-doc-type')?.value);
    });

    // When doc type selector changes, auto-adjust hints
    root.querySelector('#nob-doc-type')?.addEventListener('change', (e) => {
      const type = e.target.value;
      if (type === 'AADHAAR') {
        const cInput = root.querySelector('#nob-doc-country');
        if (cInput && (!cInput.value || cInput.value === 'United Kingdom')) {
          cInput.value = 'India';
        }
      }
    });
  ...
  runDocumentExtraction(docType = 'PASSPORT', file = null) {
    ...
    const extractBtn = root.querySelector('#btn-nob-extract-doc');
    const badge = root.querySelector('#nob-doc-status-badge');
    if (extractBtn) {
    ...
    if (badge) {
      badge.textContent = 'Optical Scan in Progress...';
      badge.parentElement.className = 'px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 font-bold text-[10px] flex items-center gap-1';
    }

    setTimeout(() => {
      const currentDocType = root.querySelector('#nob-doc-type')?.value || docType || 'PASSPORT';
      const guestFirst = root.querySelector('#nob-fname')?.value.trim() || this.data.firstName || 'Sarah';
      const guestLast = root.querySelector('#nob-lname')?.value.trim() || this.data.lastName || 'Mitchell';
      const currentCountry = root.querySelector('#nob-nationality')?.value.trim() || this.data.nationality || 'United Kingdom';

      let extractedNum = '';
      let country = currentCountry;
      let expiry = '2034-08-20';
      let dob = '1989-07-22';
      let addr = '42 Kensington Gardens, London, W8 4PE';
      ...
      if (currentDocType === 'AADHAAR') {
        ...
        extractedNum = `${rand4_1} ${rand4_2} ${rand4_3}`;
        country = 'India';
        expiry = '2099-12-31';
        dob = '1992-04-15';
        addr = '12A, Brigade Gateway, Malleshwaram, Bengaluru, Karnataka 560055';
        const natInput = root.querySelector('#nob-nationality');
        if (natInput && (!natInput.value || natInput.value === 'United Kingdom')) {
          natInput.value = 'India';
        }
      } else if (currentDocType === 'PASSPORT') {
        const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
        const pfx = chars[Math.floor(Math.random() * chars.length)];
        extractedNum = `${pfx}${Math.floor(10000000 + Math.random() * 90000000)}`;
        country = currentCountry || 'United Kingdom';
        expiry = '2033-11-12';
      } else if (currentDocType === 'DRIVERS_LICENSE') {
        extractedNum = `DL-${(currentCountry || 'UK').substring(0, 2).toUpperCase()}-${Math.floor(100000 + Math.random() * 900000)}`;
        expiry = '2031-06-30';
      } else {
        extractedNum = `ID-${Math.floor(10000000 + Math.random() * 90000000)}`;
        expiry = '2035-01-01';
      }

      // Populate form fields
      const docNumInput = root.querySelector('#nob-doc-num');
      const docCountryInput = root.querySelector('#nob-doc-country');
      const docExpiryInput = root.querySelector('#nob-doc-expiry');
      const docDobInput = root.querySelector('#nob-doc-dob');
      const docAddrInput = root.querySelector('#nob-doc-addr');

      if (docNumInput) docNumInput.value = extractedNum;
      if (docCountryInput) docCountryInput.value = country;
      if (docExpiryInput) docExpiryInput.value = expiry;
      if (docDobInput) docDobInput.value = dob;
      if (docAddrInput) docAddrInput.value = addr;
      ...
      if (badge) {
        badge.textContent = `${currentDocType} OCR Extracted`;
        badge.parentElement.className = 'px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-bold text-[10px] flex items-center gap-1';
      }

      store.showToast(`✓ Document OCR extracted: ${currentDocType} ${extractedNum} successfully verified!`, 'success');
    }, 600);
  }
}
