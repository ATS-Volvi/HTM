...
export class DocumentVerificationModal {
          ...
          ${this.scanState === 'READY' ? `
            ...
            <div class="space-y-4">
              <div class="p-4 rounded-xl bg-surface-bright border border-outline-variant/70">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  ${[
                    ...
                    { id: 'DRIVERS_LICENSE', label: 'Driver License', icon: 'directions_car' },
                    { id: 'NATIONAL_ID', label: 'National ID', icon: 'badge' }
                  ].map(t => `
                    <label class="doc-type-label border rounded-xl p-2.5 flex flex-col items-center justify-center text-center cursor-pointer transition-all ${
                      this.docType === t.id ? 'border-primary bg-primary/5 ring-1 ring-primary/20 font-bold text-primary' : 'border-outline-variant hover:bg-surface-container text-on-surface-variant'
                    }" data-doc-type="${t.id}">
                      <input type="radio" name="doc-type-sel" value="${t.id}" ${this.docType === t.id ? 'checked' : ''} class="sr-only">
                      <span class="material-symbols-outlined text-[22px] mb-1">${t.icon}</span>
                      <span class="text-[11px] leading-tight">${t.label}</span>
                    </label>
                  `).join('')}
                </div>
              </div>
              ...
              <div class="p-8 border-2 border-dashed border-outline-variant rounded-2xl flex flex-col items-center justify-center text-center bg-surface-bright/50">
                ...
                <h4 class="font-bold text-sm text-primary">Place ID Document on Scanner or Upload File</h4>
                <p class="text-[11px] text-on-surface-variant max-w-sm mt-1">
                  Ready to capture optical and chip data. Simulates 3M / Thales document scanner OCR extraction and digital ID verification.
                </p>
                <div class="flex flex-wrap items-center justify-center gap-3 mt-4">
                  <input type="file" id="input-modal-id-file" accept="image/*,application/pdf" class="hidden">
                  <button type="button" id="btn-modal-upload-id" class="px-4 py-2.5 rounded-xl border border-outline-variant bg-surface-bright hover:bg-surface-container font-bold text-xs text-primary flex items-center gap-1.5 cursor-pointer transition-all">
                    <span class="material-symbols-outlined text-[18px]">upload_file</span>
                    <span>Upload ID File</span>
                  </button>
                  <button id="btn-start-id-scan" class="px-6 py-2.5 rounded-xl bg-primary text-on-primary font-bold text-xs shadow-sm hover:bg-primary/90 flex items-center gap-2 cursor-pointer transition-all">
  ...
  bindEvents() {
    ...
    const triggerScanExecution = (file = null) => {
      this.scanState = 'SCANNING';
      this.renderContent();

      setTimeout(() => {
        const guestName = this.reservation.guestName || this.reservation.guest_name || 'Guest';
        if (this.docType === 'AADHAAR') {
          ...
          this.extractedData = {
            ...
            documentNumber: `${rand4_1} ${rand4_2} ${rand4_3}`,
            issuingCountry: 'India',
            dateOfBirth: '1992-04-15',
            expiryDate: '2099-12-31',
            extractedAddress: '12A, Brigade Gateway, Malleshwaram, Bengaluru, Karnataka 560055',
            mrzLine: `UIDAI<IND<<${guestName.toUpperCase().replace(/[^A-Z]/g, '<<')}`
          };
          ...
          this.extractedData = {
            fullName: guestName,
            documentType: 'PASSPORT',
            documentNumber: `${pfx}${Math.floor(10000000 + Math.random() * 90000000)}`,
            issuingCountry: this.reservation.nationality || 'United Kingdom',
            dateOfBirth: '1989-07-22',
            expiryDate: '2033-11-12',
            extractedAddress: '42 Kensington Gardens, London W8 4PX',
            mrzLine: `P<GBR${guestName.toUpperCase().replace(/[^A-Z]/g, '<<')}<<<<<<<<<<<<<<<<<<<\n${Math.floor(10000000 + Math.random() * 90000000)}<4GBR8907228F3205186`
          };
        } else if (this.docType === 'DRIVERS_LICENSE') {
          this.extractedData = {
            fullName: guestName,
            documentType: 'DRIVERS_LICENSE',
            documentNumber: `DL-${(this.reservation.nationality || 'UK').substring(0, 2).toUpperCase()}-${Math.floor(100000 + Math.random() * 900000)}`,
            issuingCountry: this.reservation.nationality || 'United Kingdom',
            dateOfBirth: '1990-05-18',
            expiryDate: '2031-06-30',
            extractedAddress: '15 High Street, Oxford, OX1 4DB'
          };
        } else {
          this.extractedData = {
            fullName: guestName,
            documentType: 'NATIONAL_ID',
            documentNumber: `ID-${Math.floor(10000000 + Math.random() * 90000000)}`,
            issuingCountry: this.reservation.nationality || 'United Kingdom',
            dateOfBirth: '1991-03-12',
            expiryDate: '2035-01-01',
            extractedAddress: '78 Cathedral Road, Cardiff, CF11 9HB'
          };
        }
    ...
    const uploadBtn = $('btn-modal-upload-id');
    const fileInput = $('input-modal-id-file');
    if (uploadBtn && fileInput) {
      uploadBtn.onclick = () => fileInput.click();
      fileInput.onchange = (e) => {
        const file = e.target.files?.[0];
        if (file) {
          triggerScanExecution(file);
        }
      };
    }
